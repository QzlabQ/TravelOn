...
Tasks that need services run `docker compose up -d --build`, poll the Gateway until ready, and on exit stop only what they started, leaving your existing containers alone. `mise run test:ci` reproduces the CI test stages on both Windows and Linux without needing Bash on the host.

What each stage means:

- **`pre` (17 cases)** asserts the data and configuration themselves — whether the seeded ticket window has expired, how the Flyway migrations are written, whether bundled images exist on the classpath, and the MQ queue and gateway route declarations. None of it tests business logic, so it runs first: if the data or config is wrong there is no point going further. Marked with JUnit's `@Tag("pre")`.
- **`unit` (143 cases)** holds only genuine unit tests, so the coverage gate measures only unit tests.
- **`integration` (49 Java `*IT` + 15 Agent + 33 API-through-the-gateway)** needs the service stack or a real database. Data-layer tests that hit H2 are named `*IT` and belong here, not in `unit`.
- **`resilience`** is a short pass appended after integration: the payment-timeout compensation needs `order` rebuilt with a 10-second timeout, and the community recovery case stops a service mid-run — neither can share a batch with ordinary cases. Requires `--manage-services`.
...
The progress bar counts tasks while parsing completed case counts from the logs in real time; each task ends with a line such as `✓ travel-core-service 9.2s  21 cases`, and the summary prints the total.

Coverage is aggregated by the runner into both reports above and gated: jacoco `rules` in each module's `pom.xml`, `jest.coverageThreshold` in `travel-ui/package.json`, and `PYTHON_COVERAGE_MINIMUM` in `tests/run_tests.py`. The thresholds sit about 5 points below the values measured when the gate was introduced — they exist to **prevent regressions, not to chase a number** — and should be raised once more tests land.
...
`generated_ticket_offers.csv` files.

The templates themselves are maintained by
`travel-api/scripts/generate-ticket-offers.py` (note the dash -- a different
script from the dated expander above): it fills in missing routes between city
pairs and derives business- and first-class rows for flights. Seat classes are
separate rows in this CSV -- trains already stored one row per seat class, and
flights now work the same way. Premium cabins are not attached to every flight
(roughly 45% get business, 20% get first, decided by a hash of the flight
number), which is both closer to reality and keeps the dated CSV from tripling
in size.

Prices in the templates are the baseline for a route. When the data is expanded
into dates, every (flight/train, cabin or seat class, date) fluctuates around
that baseline by up to 20%. The fluctuation is a deterministic hash rather than
a random draw, so regenerating from the same input reproduces the exact same
CSV instead of producing a 150k-line diff.

The booking window rolls relative to the day the generator runs (3 days back to
38 days ahead by default) rather than covering a fixed date range. Once the
window falls behind the calendar every transport query returns an empty result,
so `travel-api/tests/test_seed_window.py` fails in `mise run test:pre` as soon
as the committed data no longer covers today + 30 days.
...
1. Edit `travel-api/scripts/generate_dated_ticket_offers.py`:
   ...
   Each extra day adds roughly 1 MB to each generated CSV.
...
3. If Docker Compose is already running, import the data into the existing
   ...
   Replace `admin` or `travel_core_db` if they were changed in `.env`. Ticket
   seed rows are keyed by a hash of the row content (price included), so
   changing a price produces a whole new set of ids. The seed script therefore
   runs `DELETE FROM public.ticket_offer_templates` before importing --
   otherwise the old rows would stay behind and the same departure would show
   two different prices on the same day. A fresh environment gets the same
   treatment automatically through Flyway's `R__seed.sql`.
