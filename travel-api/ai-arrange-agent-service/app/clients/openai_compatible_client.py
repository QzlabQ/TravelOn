...
class OpenAICompatibleClient:
        ...
        if not self._settings.model_api_key:
            return self._tool_result(
                ...
                warnings=[
                    ToolWarning(
                        code="MODEL_DISABLED",
                        message="AI API Key 未配置，已使用本地兜底规划。",
                        source="model",
                    )
                ],
            )
        ...
        async with httpx.AsyncClient(timeout=self._settings.model_timeout_seconds) as client:
            for attempt in range(self._settings.model_retry_count + 1):
                ...
                try:
                    ...
                    if attempt < self._settings.model_retry_count:
                        await self._sleep_before_retry(attempt)
                        continue
                    warnings.append(
                        ToolWarning(
                            code="MODEL_RATE_LIMIT",
                            message="模型服务返回限流响应，已使用本地兜底规划。",
                            source="model",
                        )
                    )
                    return self._tool_result(
    ...
    def _chat_completions_url(self) -> str:
        base_url = self._settings.model_base_url.rstrip("/")
        path = self._settings.model_chat_completions_path
    ...
    def _payload_metrics(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "slowResponseWarningMs": self._settings.model_slow_response_warning_ms,
        }
        ...
        *,
        ...
        warnings = [
            ToolWarning(
                ...
                message=(
                    f"模型输出未通过结构校验：{validation_summary}；"
        ...
        except (ModelRateLimitError, httpx.TimeoutException, httpx.HTTPError, ValueError, KeyError, ValidationError, TypeError) as repair_error:
            repair_summary = format_validation_error(repair_error)
            metadata["repairError"] = repair_summary
            warnings.append(
                ToolWarning(
                    code="MODEL_OUTPUT_REPAIR_FAILED",
                    message=f"模型输出修复失败：{repair_summary}",
                    source="model",
                )
            )
            return self._tool_result(
    ...
    def _append_slow_response_warning(
        ...
        threshold_ms = self._settings.model_slow_response_warning_ms
        ...
        warnings.append(
            ToolWarning(
                ...
                message=(
                    f"模型本次响应耗时 {request_ms}ms，超过慢响应阈值 {threshold_ms}ms；"
